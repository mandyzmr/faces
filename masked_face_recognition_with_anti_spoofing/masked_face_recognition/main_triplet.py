...
def run_train(config):
    ...
    log = Logger()
    log.open(os.path.join(out_dir, f'{model_name}_triplet_training.txt'), mode='a') #增加内容
    ...
    (train_dataset, train_loader), (valid_dataset, valid_loader), (valid_masked_dataset, valid_masked_loader) = get_dataset(softmax_criterion=False, 
                        att_criterion=False, masked_face=config.masked_face, fusion_face=config.fusion_face)
    ...
    model = get_model(name=config.model, embedding_dim=EMBEDDING_DIM, n_classes=N_CLASSES, att_model=config.attention, 
                        cbam=config.cbam, softmax_criterion=False, att_criterion=False, 
    ...
    log.write(f'criterion = triplet loss\n')
    ...
    pattern1="{: ^12}|{:-^26}|{:-^60}|\n" #开头第一行
    pattern2="{: ^6}"*2+"|"+"{: ^13}"*2+"|"+"{: ^6}{: ^14}"*3+"|"+"{: ^12}\n" #标题行
    pattern3="{: ^6}"+"{: ^6.4f}"+"|"+"{: ^13}"+"{: ^13.3f}"+"|"+"{: ^6.3f}{: ^14.3f}"*3+"|"+"{: ^12}\n" #内容行
    log.write(pattern1.format('',' TRAIN ',' VALID '))
    log.write(pattern2.format('epoch','lr','hard_samples','triplet_loss', 'auc','masked_auc','acc','masked_acc',
                                'recall','masked_recall','time'))
    log.write("-"*110+'\n') 
    
    history = defaultdict(list)
    val_auc, val_masked_auc, val_acc, val_masked_acc, val_recall, val_masked_recall = 0,0,0,0,0,0 #前半周期不做validation
    start = time.time() #计时
    for e in range(epoch_start, epoch_start+config.epochs): #继续从上次的epoch训练      
        ...
        if valid_loader and valid_masked_loader and e >= config.epochs // 2: #由于前半周期肯定不断进步，只在下半周期做validation
            val_auc, val_acc, val_recall = val_on_epoch(model, valid_loader, device, e, roc_dir, masked=False)
            history['val_auc'].append(val_auc)
            history['val_acc'].append(val_acc)
            history['val_recall'].append(val_recall)
            
            val_masked_auc, val_masked_acc, val_masked_recall = val_on_epoch(model, valid_masked_loader, device, e, roc_dir, masked=True)
            history['val_masked_auc'].append(val_masked_auc)
            history['val_masked_acc'].append(val_masked_acc)
            history['val_masked_recall'].append(val_masked_recall)
            end = time.time() #每个epoch结束后计算一次累计时间
            log.write(pattern3.format(e+1, lr, n_hard, train_triplet_loss, val_auc, val_masked_auc, 
                                      val_acc, val_masked_acc, val_recall, val_masked_recall, time_to_str(end - start)))
    ...
    pickle.dump(history, open(os.path.join(out_dir, f'{model_name}_triplet_history.pkl'),'wb'))
